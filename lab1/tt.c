    ...
    while (fgets(input, LENGTH_LIMIT, stdin)) {
        ...
        if (np_cnt) {
            for (i = 0; i < np_cnt; i++) {
                ...
                // number test.html | number | removetag |2 removetag test.html | removetag | 1


                //! removetag test.html |2 removetag test.html |1做完再做number會炸掉?????????????????????
                minus_ttl(&knowNode);
                targetNode = getSpecificNode(&knowNode, 0);
                if (ifNumberPipe) {
            ...
            addInfo2ExistNode(newNode, &tmp_op, OP_len);
            adoptChild(newNode, targetNode);
            bin_func(&tmp_op, filename, ifNumberPipe, newNode);
        } else {
...
int split_terms(char *input, ordinary_pipe_t *op, char *filename) {
    ...
    // for (i = 0; i < op->cnt; i++) {
    //     printf("%d, ", op->cmds[i].cnt);
    //     for (j = 0; j < op->cmds[i].cnt; j++) {
    //         printf("%s\t", op->cmds[i].terms[j]);
    //     }
    //     printf("|\t");
    // }
    // printf("\n");
...
// Return end index
int copy_ith_op(ordinary_pipe_t *tmp_op, ordinary_pipe_t *current_op, int start_index) {
    int i, j;
    bool first_flg;
    cmd *tmpCmd, *localCmd;
