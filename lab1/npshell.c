...
void split_terms(char *, char *[LENGTH_LIMIT][TERM_AMT_LIMIT], int *, int *, char *);
void show(char *[LENGTH_LIMIT][TERM_AMT_LIMIT], int, int *);
void printenv(char **, int);
void bin_func(char *[LENGTH_LIMIT][TERM_AMT_LIMIT], int, int *, char *, bool, node *);
void trim(char *);
...
void addInfo2ExistNode(node *, char *[LENGTH_LIMIT][TERM_AMT_LIMIT], int[], int);
void ordinary_pipe(char *[LENGTH_LIMIT][TERM_AMT_LIMIT], int);
bool check_valid(char *[LENGTH_LIMIT][TERM_AMT_LIMIT], int, char *, bool *, int *);
...
void split_terms(char *input, char *terms[LENGTH_LIMIT][TERM_AMT_LIMIT], int *cmd_amt, int *terms_amt, char *filename) {
    char d[10];
    char *p, *commands;
...
void show(char *terms[LENGTH_LIMIT][TERM_AMT_LIMIT], int cmd_amt, int *terms_amt) {
    int x, y;
...
void bin_func(char *terms[LENGTH_LIMIT][TERM_AMT_LIMIT], int cmd_amt, int *terms_amt, char *filename, bool ifNumberPipe, node *Node) {
    int fd;
    pid_t pid;
    pid = fork();
...
void addInfo2ExistNode(node *Node, char *inputCmd[LENGTH_LIMIT][TERM_AMT_LIMIT], int terms_amt[], int cmd_amt) {
    cmd *localCmd;
    int cmd_idx, i;
...
bool check_valid(char *terms[LENGTH_LIMIT][TERM_AMT_LIMIT], int cmd_amt, char *illegal_term, bool *ifNumberPipe, int *OP_len) {
    int i, j;
    bool legal_flg;
...
bool check_numbered_pipe(char *terms[LENGTH_LIMIT][TERM_AMT_LIMIT], int cmd_amt) {
    int i, j;
    for (i = 0; i < cmd_amt; i++) {
        char p[LENGTH_LIMIT];
        strcpy(p, terms[i][0]);
